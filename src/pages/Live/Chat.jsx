...
export const chatHeightSetting = `xl:h-[605px] md:h-[495px]`;
      ...
      >
        <div className="px-4 md:py-3 py-1">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center justify-center text-[var(--idol-text-color)] font-semibold gap-3 overflow-hidden min-w-0">
            ...
            <div>
              {show ? (
                <button type="button" className="m-1 cursor-pointer">
                  <IoMdArrowDropup
                    className="text-[var(--idol-text-color)] text-2xl cursor-pointer"
                    onClick={toggleShow}
                  />
                </button>
              ) : (
...
function WarningAndPinnedComment() {
  const message = `Chào mừng bạn đến với phòng phát trực tiếp của 789BET Live thiết lập
  ...
  return (
    <div
      className={`text-white flex flex-col p-2 ${chatHeightSetting} overflow-auto ${
        isMobileKeyboardOpen ? "h-[25dvh]" : "h-[50dvh]"
      }`}
    >
                  ...
                  <div className="flex gap-1">
                    <div
                      className={`flex gap-0.5 text-[var(--color-brand-primary)] text-sm font-medium mb-1 ${
                        isSpecial ? "text-[var(--idol-text-color)]" : ""
                      }`}
                    >
                    ...
                    <div
                      className={`text-gray-300 text-xs leading-relaxed ${
                        isSpecial
                          ? "!text-[var(--idol-text-color)] font-semibold"
                          : ""
                      } break-all`}
                    >
                      {comment.message}
...
function ChatInterface({ className = "", hideInput = false }) {
  return (
    <div
      className={`flex flex-col h-full w-full justify-between ${chatHeightSetting} bg-[var(--lighter-background)] ${className}`}
    >
      <ChatFrame />
      {!hideInput && (
