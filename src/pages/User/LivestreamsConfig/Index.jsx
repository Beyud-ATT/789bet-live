...
const LivestreamConfigForm = () => {
  ...
  return (
    <>
      ...
      <div className="md:w-[80%] w-full h-full md:bg-[#0C111D] md:py-8 px-4 lg:mx-0 mx-auto">
        <DeviceProvider.MOBILE>
          <Divider className={`${formStyle} !border-white`} />
        </DeviceProvider.MOBILE>
...
function LivestreamConfigInfoForm({ livestreamData, isLoading }) {
  ...
  const rtmpOptions = rtmpServers?.data?.data?.map((server) => ({
    value: server.id,
    label: <span className="text-gray-300">{server.serverName}</span>,
  }));
  ...
  useEffect(() => {
    if (
      ...
      if (livestreamData) {
        form.setFieldsValue({
          ...livestreamData,
          rtmpServerId: livestreamData?.rtmpServerId,
        });
      }
    }
  }, [
    ...
    <div className={`${formStyle}`}>
      ...
      >
        ...
        <Form.Item
          ...
          label={<span className="text-white">Hình đại diện</span>}
        >
          <Flex gap={10}>
        ...
        <Form.Item
          name="rtmpServerId"
          label={<span className="text-white">RTMP Server</span>}
        >
  ...
  return (
    <div className={`${formStyle} mt-8`}>
      ...
      >
        <Form.Item
          name="scheduleTime"
          label={<span className="text-white">Lập lịch hẹn livestream</span>}
        >
          <DatePicker
            showTime
        ...
        {sessionData && (
          <>
            <Form.Item
              label={<span className="text-white">Server Stream</span>}
            >
              <ServerStreamField value={sessionData?.rtmpEndpoint} />
            </Form.Item>

            <Form.Item label={<span className="text-white">Stream key</span>}>
