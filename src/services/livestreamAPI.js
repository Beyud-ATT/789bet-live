...
async function addBannedChat(data) {
  try {
    const res = await axoisBase.post(`${endpoint}/chat/me/hidden`, data);
    return res;
  } catch (error) {
...
async function unBannedChat(data) {
  try {
    const res = await axoisBase.post(`${endpoint}/chat/me/open`, data);
    return res;
  } catch (error) {
...
async function checkChat(data) {
  try {
    const res = await axoisBase.get(`${endpoint}/chat/me/list`, {
      params: data,
    });
    return res;
  } catch (error) {
